...
def send_password_reset_email(request, user_id):
    ...
    to_email = user.email  # O endereço de e-mail do usuário
    template_name = 'registration/password_reset_email.html'

    # Gera o token para o processo de redefinição de senha
    token = default_token_generator.make_token(user)
    uid = urlsafe_base64_encode(force_bytes(user.pk))

    # Contexto a ser passado para o template de email
    context = {
        'protocol': 'https',
        'domain': 'saudebras.com.br',
        'uid': uid,
        'token': token,
        'user': user,
    }

    # Renderiza o e-mail HTML
    html_content = render_to_string(template_name, context)
    text_content = strip_tags(html_content)  # Versão texto para compatibilidade com clientes de e-mail

    # Cria o objeto de e-mail com alternativas para texto puro e HTML
